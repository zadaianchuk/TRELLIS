...
class NoisyImageConditionedSparseFlowMatchingCFGTrainer(NoisyImageConditionedMixin, SparseFlowMatchingCFGTrainer):
    """
    Trainer for sparse image-conditioned diffusion model with RAE-style noisy conditioning.
...
# ============================================================================
# LoRA-enabled sparse trainers
# ============================================================================
from .mixins.lora_mixin import LoRAMixin


class LoRAImageConditionedSparseFlowMatchingCFGTrainer(LoRAMixin, ImageConditionedSparseFlowMatchingCFGTrainer):
    """
    LoRA-enabled trainer for sparse image-conditioned diffusion with flow matching.
    
    Uses Low-Rank Adaptation (LoRA) for efficient fine-tuning of sparse structure
    flow models with significantly fewer trainable parameters.
    
    Reference: https://github.com/RuiningLi/dso
    
    Additional Args (beyond ImageConditionedSparseFlowMatchingCFGTrainer):
...
class LoRANoisyImageConditionedSparseFlowMatchingCFGTrainer(LoRAMixin, NoisyImageConditionedSparseFlowMatchingCFGTrainer):
    """
    LoRA-enabled trainer for noisy sparse image-conditioned diffusion with flow matching.
    
    Combines LoRA adapters with noisy image conditioning for robust fine-tuning
    of sparse structure flow models.
    
    Additional Args (beyond NoisyImageConditionedSparseFlowMatchingCFGTrainer):
        use_lora (bool): Whether to use LoRA adapters. Default: False.
        lora_r (int): LoRA rank. Default: 64.
        lora_alpha (int): LoRA scaling factor. Default: 128.
        lora_dropout (float): LoRA dropout rate. Default: 0.0.
